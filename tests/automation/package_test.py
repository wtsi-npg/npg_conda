...
class TestPackage(object):
    ...
    @m.it("Can get sub-package names from a RecipeBook")
    ...
    @m.it("Can find test scripts created at build time")
    ...
    @m.it("Can detect success or failure of package test scripts")
    def test_run_test_scripts(self, mocker):
        ...
        mocker.patch('automation.package.Package.get_test_scripts',
                     return_value=[str(conda_path) +
                                   '/pkgs/success-1.0.0-0/info/'
                                   'test/run_test.fake'])
        with pytest.raises(ValueError) as err:
            success.run_test_scripts(env)
        assert str(err.value) == 'Unsupported test extension: fake'

    @m.it("Can find system versions of the package libraries being used "
          "instead of conda versions")
    def test_ldd(self, mocker):
        mocker.patch('automation.package.run_command',
                     return_value=(
                         "libsub.so.0 => /usr/lib/libsub.so.0 "
                         "(0x00007ffee9523000)",
                         "", 0))
        with pytest.raises(package.LibError) as err:
            sub.check_ldd("path", "env")
        assert str(
            err.value) == "libsub.so.0 => /usr/lib/libsub.so.0 " \
                          "(0x00007ffee9523000)"
